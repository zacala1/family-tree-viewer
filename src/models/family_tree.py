...
class FamilyTree:
    """Family tree manager with thread-safe operations.

    Manages all persons and relationships in the family tree.
    Uses RLock for thread-safe concurrent access protection.
    """
    ...
    def __init__(self):
        ...
        self._spouse_rel_index: Dict[frozenset, str] = {}  # frozenset({p1,p2}) -> rel_id
    ...
    def add_relationship(self, relationship: Relationship) -> None:
        ...
        with self._lock:
            ...
            if relationship.rel_type == RelationType.SPOUSE:
                key = frozenset({relationship.person1_id, relationship.person2_id})
                self._spouse_rel_index[key] = relationship.id
            self._modified = True
    ...
    def _would_create_cycle(self, ancestor_id: str, descendant_id: str) -> bool:
        ...
        Returns True if descendant is already an ancestor of ancestor_id.
        Uses BFS with visited set to detect actual cycles (not just depth limit).
        """
        ...
        while queue:
            ...
            # Check if we reached the descendant (cycle detected)
            ...
            # Safety check: if visited set becomes too large, likely indicates a problem
            if len(visited) > MAX_CYCLE_DEPTH:
                from ..utils.logger import warning
                warning(
                    f"Cycle detection visited {len(visited)} nodes. "
                    f"This may indicate a very deep family tree or a data issue."
                )
                # Continue checking rather than returning True (false positive prevention)

            person = self.get_person(current_id)
            if person:
                # Add parents to queue if not yet visited
                if person.father_id and person.father_id not in visited:
                    queue.append(person.father_id)
                if person.mother_id and person.mother_id not in visited:
                    queue.append(person.mother_id)
    ...
    def get_spouse_relationship(self, person1_id: str, person2_id: str) -> Optional[Relationship]:
        ...
        with self._lock:
            key = frozenset({person1_id, person2_id})
            rel_id = self._spouse_rel_index.get(key)
            if rel_id:
                return self._relationships.get(rel_id)
            return None
    ...
    def calculate_generations(self, force: bool = False) -> None:
        """Calculate generation information for all persons using BFS.
